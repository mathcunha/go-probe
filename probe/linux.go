/*
this code was based/copied/merged from rtop - the remote system monitoring utility
https://github.com/rapidloop/rtop/blob/master/stats.go#L331
*/
package probe
...
func runCommand(command string, arg ...string) (stdout string, err error) {
	...
	if err != nil {
		log.Printf("runCommand: %s:%v", command, err)
		return
	}
	stdout = string(out)
	//log.Printf("runCommand: stdout - %v\n", stdout)
...
func getUptime(stats *Stats) (err error) {
	uptime, err := runCommand("/bin/cat", "/proc/uptime")
	if err != nil {
		return
	}
...
func getHostname(stats *Stats) (err error) {
	hostname, err := runCommand("/bin/hostname", "-f")
	if err != nil {
		return
	}
...
func getMemInfo(stats *Stats) (err error) {
	lines, err := runCommand("/bin/cat", "/proc/meminfo")
	if err != nil {
		return
	}
...
func getFSInfo(stats *Stats) (err error) {
	lines, err := runCommand("/bin/df", "-B1")
	if err != nil {
		return
	}
...
func getInterfaces(stats *Stats) (err error) {
	var lines string
	lines, err = runCommand("/bin/ip", "-o", "addr")
	if err != nil {
		// try /sbin/ip
		lines, err = runCommand("/sbin/ip", "-o", "addr")
		if err != nil {
			return
		}
	}
...
func getInterfaceInfo(stats *Stats) (err error) {
	lines, err := runCommand("/bin/cat", "/proc/net/dev")
	if err != nil {
		return
	}
...
func getLoad(stats *Stats) (err error) {
	line, err := runCommand("/bin/cat", "/proc/loadavg")
	if err != nil {
		return
	}
...
func getCPU(stats *Stats) (err error) {
	lines, err := runCommand("/bin/cat", "/proc/stat")
	if err != nil {
		return
	}
